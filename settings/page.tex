\renewcommand*\familydefault{\sfdefault} % Comment out for serif font
\RedeclareSectionCommand[beforeskip=15pt, afterskip=5pt]{chapter}
\RedeclareSectionCommand[beforeskip=5pt, afterskip=2pt]{section}

% Seitenränder
\RequirePackage[left=2cm,right=2cm,top=2cm,bottom=2cm,headheight=1cm,headsep=0.5cm]{geometry}

% Zielenabstand 1,5
\RequirePackage{setspace}
\onehalfspacing

%Header
\RequirePackage{scrlayer-scrpage}
\ohead*{
    \includegraphics[height=20pt]{image/Nordakademie_Logo_small}
}

\titlehead{\includegraphics[width=\textwidth]{image/Nordakademie_Logo_gross}}

\newcommand{\origBackmatter}{\backmatter}
\renewcommand{\backmatter}{\clearpage\pagenumbering{Roman}\origBackmatter}